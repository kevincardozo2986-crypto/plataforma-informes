...
def build():
    ...
    for index, (title, subtitle, sections) in enumerate(PAGES):
        ...
        blocks.append('<table width="100%%" bgcolor="#103D6C" cellpadding="18"><tr><td><font color="#F3CA65">GUÍA DE USO · USTA</font><h1 style="color:white">%s</h1><p style="color:#DBEAFA">%s</p></td></tr></table>' % (escape(title), escape(subtitle)))
        if index == 0:
        ...
        blocks.append('<p style="color:#718096;font-size:9pt">Plataforma de Informes USTA · Manual de usuario · %s / %s</p></div>' % (index+1, len(PAGES)))
    ...
    writer.setTitle('Manual de usuario · Plataforma de Informes USTA')
    writer.setCreator('Plataforma de Informes USTA')
